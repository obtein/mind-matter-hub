...
import { usePatientDetail } from "@/viewmodels/usePatientDetail";
...
export const PatientDetailView = ({ patientId, onBack, onAppointmentSelect }: PatientDetailViewProps) => {
  const vm = usePatientDetail(patientId);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        ...
        <CardContent>
          {(() => {
            const completedAppointments = vm.appointments.filter(a => a.status === "completed");
            const scheduledAppointments = vm.appointments.filter(a => a.status === "scheduled" && new Date(a.appointment_date) > new Date());

            // First visit date
            const firstVisit = vm.appointments.length > 0
              ? vm.appointments[vm.appointments.length - 1]
              : null;
            ...
            const lastPrescriptionDate = vm.medications.length > 0 && vm.medications[0].appointment_date
              ? vm.medications[0].appointment_date
              : null;
            ...
            <CardTitle className="flex items-center gap-2">
              ...
              {vm.medications.length > 0 && (
                <span className="text-sm font-normal text-muted-foreground">
                  ({vm.filteredDiagnoses.length}/{vm.medications.length})
                </span>
              )}
            </CardTitle>
            {vm.medications.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  <Input
                    ...
                    value={vm.diagSearchTerm}
                    onChange={(e) => vm.setDiagSearchTerm(e.target.value)}
                    className="pl-8 h-8 w-36 text-sm"
                  />
                </div>
                <Input
                  type="month"
                  value={vm.diagDateFilter}
                  onChange={(e) => vm.setDiagDateFilter(e.target.value)}
                  className="h-8 w-36 text-sm"
                  placeholder="Ay seç"
                />
                {(vm.diagSearchTerm || vm.diagDateFilter) && (
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => { vm.setDiagSearchTerm(""); vm.setDiagDateFilter(""); }}
                    className="h-8 px-2"
                  >
        ...
        <CardContent>
          {vm.medications.length === 0 ? (
            ...
            <div className="space-y-3">
              {vm.filteredDiagnoses.map((med, index) => (
                <div
                  key={med.id}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={vm.isDialogOpen} onOpenChange={vm.setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <form onSubmit={(e) => { e.preventDefault(); vm.createAppointment(e); }} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="appointment_date"
                      type="date"
                      value={vm.formData.appointment_date}
                      onChange={(e) => vm.setFormData({ ...vm.formData, appointment_date: e.target.value })}
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      id="appointment_time"
                      type="time"
                      value={vm.formData.appointment_time}
                      onChange={(e) => vm.setFormData({ ...vm.formData, appointment_time: e.target.value })}
                      required
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    value={vm.formData.duration_minutes}
                    onValueChange={(value) => vm.setFormData({ ...vm.formData, duration_minutes: value })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={vm.formData.reminder_time}
                    onValueChange={(value) => vm.setFormData({ ...vm.formData, reminder_time: value })}
                  >
                    <SelectTrigger>
                      <div className="flex items-center gap-2">
        ...
        {vm.appointments.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <Button variant="outline" className="mt-4" onClick={() => vm.setIsDialogOpen(true)}>
                <Plus className="w-4 h-4 mr-2" />
          ...
          <div className="space-y-3">
            {vm.appointments.map((appointment, index) => (
              <Card
                key={appointment.id}
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${vm.getStatusColor(appointment.status)}`}>
                        {vm.getStatusLabel(appointment.status)}
                      </span>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="opacity-0 group-hover:opacity-100 transition-opacity text-destructive hover:text-destructive"
                        onClick={(e) => { e.stopPropagation(); vm.setDeleteAppointmentId(appointment.id); }}
                      >
        ...
        <AlertDialog open={!!vm.deleteAppointmentId} onOpenChange={(open) => !open && vm.setDeleteAppointmentId(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                onClick={() => { if (vm.deleteAppointmentId) vm.deleteAppointment(vm.deleteAppointmentId); vm.setDeleteAppointmentId(null); }}
              >
                Sil
              </AlertDialogAction>
            </AlertDialogFooter>
